...
import org.freshwaterlife.portlets.search.resulttypes.ImageArchive;
...
public class ProcessResults implements Serializable {
    ...
    private int iNumOfImageArchiveResults;
	...
	try {
			...
			while (fieldsIt.hasNext()) {
				...
				while (doctypesIt.hasNext()) {
					...
					while (totalsIt.hasNext()) {
					    ...
					    } else if (total.getAttributeValue("name").compareTo("imagearchive") == 0) {
						iNumOfImageArchiveResults = Integer.parseInt(total.getValue());
		...
		while (dcs.hasNext()) {
			...
			r = new Image(docVals);
			results.add(r);
		    } else if (doctype.equalsIgnoreCase(Global.arrCores[Global.IMAGEARCHIVE])) {
			r = new ImageArchive(docVals);
			results.add(r);
    ...
    public int getINumOfImageArchiveResults() {
	return iNumOfImageArchiveResults;
    }

    public void setINumOfImageArchiveResults(int iNumOfImageArchiveResults) {
	this.iNumOfImageArchiveResults = iNumOfImageArchiveResults;
    }
