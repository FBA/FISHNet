function convertFromGridRef()
{
	
	try
	{
		latLon = OSGridToLatLong(gob('metadataForm:gridReference').value);
		clearMap();
		point = new Object();
		point.latLng = new google.maps.LatLng(latLon.lat,latLon.lon);
		addLatLng(point);
		//setstartMarker(new google.maps.LatLng(latLon.lat,latLon.lon));
		temp = new Object();
		temp.latLng = new google.maps.LatLng(latLon.lat,latLon.lon);
		addLatLng(temp);
		map.setCenter(temp.latLng);
		gob('metadataForm:northBoundingLatitude').value = point.latLng.lat().toFixed(6);
		gob('metadataForm:southBoundingLatitude').value = point.latLng.lat().toFixed(6);
		gob('metadataForm:eastBoundingLongitude').value = point.latLng.lng().toFixed(6);
		gob('metadataForm:westBoundingLongitude').value = point.latLng.lng().toFixed(6);
	}
	catch (e)
	{
		alert(e.toSource());
